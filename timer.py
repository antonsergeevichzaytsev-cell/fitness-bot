...
Четыре независимые проверки за один прогон:
...
3. Напоминание о смене фазы периодизации: session.
   ...
   шкалой.
4. Тихая фиксация пропуска: session.check_and_mark_silent_skip() —
   если предыдущий тренировочный день (program.previous_training_date)
   прошёл без записанной тренировки и без явной команды 'болею',
   помечается автоматически (reason=''). Не шлёт сообщение в Telegram
   — только для полноты данных под будущую сводку/анализ. Идемпотентна,
   безопасно вызывать каждый прогон.

Все четыре взаимоисключающи по большей части на практике, но проверяются
...
def main():
    ...
    # Тихая фиксация пропуска — не шлёт сообщение (не хотим спамить
    # уведомлением о том, что вчера ничего не было, это просто для
    # полноты данных под будущую сводку). Идемпотентна — is_day_skipped
    # проверка внутри не даёт пометить повторно, безопасно вызывать
    # каждую минуту, не только раз в день.
    if sess.check_and_mark_silent_skip(data):
        did_something = True
        print("Silent skip marked for previous training day.")
